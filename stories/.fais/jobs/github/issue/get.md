...
2. **Create issue directory**: Create a directory named `github_issue_<number>` in the story
...
3. **Fetch issue data**: Use the
   `gh issue view <number> --json number,title,state,url,body,author,createdAt,comments` command to
   retrieve:
   - Issue title
...
4. **Download attachments**: If the issue description or comments contain images or file
   attachments:
...
5. **Create issue.md**: Generate a markdown file named `issue.md` inside the issue directory with:
   ...
   - Issue metadata (number, author, state, created date, URL)
   - Issue description/body
   - All comments in chronological order, clearly formatted with commenter name and timestamp

6. **Update file references**: Replace any GitHub-hosted image URLs or file URLs in the markdown
   with relative paths to the locally downloaded files.
